...
def add_eval_args(parser: argparse.ArgumentParser):
    parser.add_argument("--saved_model_dir", type=str,
                        default="D:\\Reproduction_of_the_paper\\light_mappo\\results\\MyEnv\\MyEnv\\mappo\\check\\run1\\models",
...
def evaluate_under_attack(runner, args) -> dict:
    envs = runner.envs
    ...
    while finished_eps < args.n_eval_episodes:
        ...
        if env_done:
            ...
            if (finished_eps % LOG_INT == 0) or (finished_eps == args.n_eval_episodes):
                ...
                f = open("./eval_without_attack.csv", "a+", newline="", encoding="utf-8-sig")
                print(
...
def main():
    ...
    # 评测
    stats = evaluate_under_attack(runner, args)
